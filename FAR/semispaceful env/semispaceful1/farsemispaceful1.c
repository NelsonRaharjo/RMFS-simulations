...
void setup_agents() {
    agents[0] = (Agent){'A', {1, 1}, {8, 8}, {1, 1}, false, 0};
    agents[1] = (Agent){'B', {1, 7}, {8, 0}, {1, 7}, false, 0};
    agents[2] = (Agent){'C', {7, 1}, {0, 8}, {7, 1}, false, 0};
    agents[3] = (Agent){'D', {7, 7}, {0, 0}, {7, 7}, false, 0};
    agents[4] = (Agent){'E', {3, 1}, {5, 8}, {3, 1}, false, 0};
...
int main() {
    ...
    for (int i = 0; i < agent_count; i++) {
        if (!agents[i].done) {
            printf("Agent %c failed to reach goal\n", agents[i].id);
            all_done = false;
        }
    }
    if (all_done) {
