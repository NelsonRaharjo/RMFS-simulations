...
// Add a 5th direction for "wait" (stay in place)
int dRow[] = {-1, 1, 0, 0, 0};
int dCol[] = {0, 0, -1, 1, 0};
...
// Each agent has its own constraint table: step -> [row][col]
...
int has_conflict(Position a_pos, int a_step, Position b_pos, int b_step, Position a_prev, Position b_prev) {
    ...
    if (a_pos.row == b_pos.row && a_pos.col == b_pos.col) return 1; // vertex
    if (a_prev.row == b_pos.row && a_prev.col == b_pos.col &&
        b_prev.row == a_pos.row && b_prev.col == a_pos.col) return 1; // swap
    return 0;
}
...
Node* a_star_search(int agent_id, int local_constraints[MAX_STEPS][MAX_ROWS][MAX_COLS]) {
    ...
    while (open_size > 0) {
        ...
        if (current->pos.row == agents[agent_id].goal.row &&
            ...
            // Extend path to hold at goal for all remaining steps
        ...
        for (int i = 0; i < 5; i++) { // 5 directions: up, down, left, right, wait
            Position next_pos = { current->pos.row + dRow[i], current->pos.col + dCol[i] };
            // For "wait", don't move out of bounds or into a wall
...
void visualize_timestep(int step) {
    static int goal_reported[MAX_AGENTS] = {0}; // Track if agent's goal was already reported
    static int goal_time[MAX_AGENTS] = {0};     // Track when each agent reached its goal
    printf("Timestep %d:\n", step);
    for (int r = 0; r < grid.rows; r++) {
        for (int c = 0; c < grid.cols; c++) {
            char cell = grid.grid[r][c];
    ...
    // Warn if any agent moves onto a finished agent's goal coordinate
    ...
    // Notify only the first time an agent reaches its goal
    for (int i = 0; i < num_agents; i++) {
        if (!goal_reported[i] &&
    ...
    // Warn if two or more agents occupy the same cell at this timestep
    for (int i = 0; i < num_agents; i++) {
...
void add_goal_occupation_constraints() {
    // For each agent, prevent all other agents from entering its goal cell after it reaches the goal
...
void cbs() {
    ...
    // Add goal occupation constraints and replan if needed
...
int main() {
    ...
    num_agents = 9;
    agents[0].start = (Position){1, 1};agents[0].goal = (Position){6, 3};
    agents[1].start = (Position){1, 3};agents[1].goal = (Position){6, 1};
    agents[2].start = (Position){1, 5};agents[2].goal = (Position){3, 6};
    agents[3].start = (Position){3, 1};agents[3].goal = (Position){6, 5};
    agents[4].start = (Position){3, 3};agents[4].goal = (Position){6, 0};
    agents[5].start = (Position){3, 5};agents[5].goal = (Position){3, 0};
    agents[6].start = (Position){5, 1};agents[6].goal = (Position){0, 4};
    agents[7].start = (Position){5, 3};agents[7].goal = (Position){0, 0};
    agents[8].start = (Position){5, 5};agents[8].goal = (Position){0, 2};
