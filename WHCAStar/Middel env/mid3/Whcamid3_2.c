...
void print_grid(Agent agents[], int agent_count, int time) {
    ...
    for (int a = 0; a < agent_count; a++) {
        display[agents[a].goal.y][agents[a].goal.x] = '+';
    }
    ...
    for (int a = 0; a < agent_count; a++) {
        if (time < agents[a].path.length) {
            ...
            if (occupied[y][x]) {
                ...
                // Print all agents' previous positions
                printf("  Previous positions at time %d:\n", time > 0 ? time-1 : 0);
                for (int k = 0; k < agent_count; k++) {
                    int prev_x, prev_y;
                    if (time > 0 && time-1 < agents[k].path.length) {
                        prev_x = agents[k].path.pos[time-1].x;
                        prev_y = agents[k].path.pos[time-1].y;
                    } else {
                        prev_x = agents[k].start.x;
                        prev_y = agents[k].start.y;
                    }
                    printf("    Agent %c: (%d,%d)\n", agents[k].name, prev_x, prev_y);
                }
            }
